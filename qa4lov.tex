...
\title{Towards a QA System Over a Vocabulary Catalogue}
...
However, users need to have a minimal knowledge of SPARQL language and RDF skills to query  RDF datasets. It is one barrier that is overcome by Question Answering (QA) systems, which directly take as input questions in natural language. The need for more advanced tools and QA systems that operate over large repositories of Linked Data has also been the motivation for the QALD Question Answering over Linked Data (QALD) series of workshops \cite{lopezetal2013}. Started in 2011, QALD challenges offer a test sets questions whose answer can be found in the DBpedia and MusicBrainz, with more complex questions added each year. 
 
Vocabulary catalogues and semantic search engines are special datasets as they classes and properties used to model and build datasets available in the Linked Data space. Ontologies are important part in the semantic web layer to build full interoperable datasets. Most vocabulary catalogues provide terms search and APIs. Linked Open Vocabulary (LOV) provides five types of search criteria: metadata search, ontology search, APIs access, bulk file and SPARQL endpoint access \cite{vandenbusschelov}.
...
Each version of a vocabulary in LOV contains metadata information which are the following:
\begin{itemize}
\item A preferred namespace; 
